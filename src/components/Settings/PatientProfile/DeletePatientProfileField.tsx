...
const DeletePatientProfileField: FC<ParamsType> = ({ id }) => {
  const { can } = useCan();
  if (!can([UserPermissionsEnum.PATIENT_PROFILE_DELETE])) return <></>;

  const { mutate, isLoading } = useCustomMutation(
    [REACT_QUERY_IDS.SETTINGS.SUPPLIERS],
    patientProfile.deleteOne
  );
...
export default DeletePatientProfileField;
